...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!path1 || !path2 || !path3) {
        ...
        paths.forEach((path)=>{
            if (path) {
                ...
                console.log(`\u{2705} ${path.id} setup complete!`);
            }
        });
    }
    ...
    function animateBlocks(activeBlock, activePath) {
        ...
        blocks.forEach((block)=>{
            if (block === activeBlock) // Fade in the active block
            tl.to(block, {
                opacity: 1,
                duration: 0.5,
                ease: "power2.out"
            }, 0); // Start at the same time
            else // Fade out inactive blocks
            tl.to(block, {
                opacity: 0,
                duration: 0.5,
                ease: "power2.out"
        ...
        } else console.warn("\u26A0\uFE0F Some paths are missing, skipping line animation");
    }
...
},{}]},["3O61n","6rimH"], "6rimH", "parcelRequirebd5d", {})
